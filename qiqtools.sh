...
check_IPV4(){
  ...
  [[ -n "$IP4_INFO" ]] && return 1; 
    ...
    local res_ipv4=$(curl -4 -sS --retry 2 --max-time 1 https://www.cloudflare.com/cdn-cgi/trace)
		...
		local local_isp4=$(curl -s -4 --max-time 3  --user-agent "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/80.0.3987.87 Safari/537.36" "https://api.ip.sb/geoip/${local_ipv4}" | grep organization | cut -f4 -d '"')
...
check_IPV6(){
  ...
  [[ -n "$IP6_INFO" ]] && return 1;
    ...
    local res_ipv6=$(curl -6 -sS --retry 2 --max-time 1 https://www.cloudflare.com/cdn-cgi/trace)
		...
		local local_isp6=$(curl -s -6 --max-time 3 --user-agent "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/80.0.3987.87 Safari/537.36" "https://api.ip.sb/geoip/${local_ipv6}" | grep organization | cut -f4 -d '"')
...
txtn "\t${pink} ${white} reinstall.sh ubuntu 24.04 [--minimal]"
txtn "\t${pink} ${white} reinstall.sh alpine 3.17|3.18|3.19|3.20"
txtn "\t${pink} ${white} reinstall.bat windows --image--name='windows server 2022 serverdatacenter' ${plain}"
txtn "\t${pink} ${white}                       --lang zh-cn ${plain}\n"
txtn "\t${pink} ${white} bash reinstall.sh windows --image-name 'Windows 10 Enterprise LTSC 2021'"
txtn "\t${pink} ${white}                           --lang zh-cn ${plain}\n"
txtn "—————————————————————————————————————"
}
